...
if file_maxi and file_market:
    ...
    st.dataframe(result_df)  # Visualizza il DataFrame nel browser
    
    # Aggiungi un'opzione per scaricare il file Excel con i risultati
    st.write("Scarica i risultati in formato Excel:")
    result_excel = result_df.to_excel(index=False)
    st.download_button(label="Scarica Excel", data=result_excel, file_name="risultati_confronto.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
